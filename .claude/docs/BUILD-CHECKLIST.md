...
- [x] **A69 — The report in the plugin's palette (2026-09-26).**
  ...
  0.28.37). Real: run 81's report read in a dark-scheme browser, still light.
- [x] **A68 — The full report, foldable and in order (2026-09-26).** Every
  ...
  technical ones folded after; the article folds per page; tables fit; steps
  named, not keyed; French status in the eyebrow. `tests/test-report.php`.
  Real: run 81's report read on desktop and phone, every fold opened, a TOC
  link to a folded section opens it.
...
- [x] **A60 — The collage leads the recipe (2026-09-25).** Scenario 1:
  ...
  `tests/test-collage-lead.php`, `tests/test-edited.php`. Real: runs 71–77
  on the test site — the writer's collage kept (potée, quiche au poulet),
  unticked (tarte aux figues), text + photo (quiche), text only (gratin,
  quiche au chèvre), photo only (rôti Orloff); the pairing checkbox ticked
  and unticked in a browser, desktop and phone; every screen swept as
  admin and writer.
