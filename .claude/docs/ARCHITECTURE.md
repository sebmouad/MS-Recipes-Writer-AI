...
| Réglages | manager | keys and a free check that each one works, ceilings, the article (language, length, pages, JSON-LD), the Facebook collage's style references (`uploads/msrwa/style/`, up to three, the first used; with none uploaded, the one shipped in `assets/style/facebook-reference.jpg`), retention, and whether the machinery is running |
