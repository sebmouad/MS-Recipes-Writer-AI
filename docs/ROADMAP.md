...
- Articles and Jobs lists: full pagination, search, publication/quality/state/
  ...
  job record per row.
- Offline test suite plus CI on PHP 7.4/8.1/8.3.
...
3. **Per-row job actions.** Retry and cancel are only reachable from the batch
   detail screen. Surface them in the Jobs list (REST endpoints already exist)
   with the same confirmation rules.
4. **Queue visibility for editors.** `MSRWA_Queue::health()` is only on the
   Configuration screen. Show a banner on the main screen when jobs are
   waiting with no scheduler progress, so a stalled cron is not read as a
   plugin failure.
5. **Migration cost.** The quality backfill runs in 200-row chunks during
   `install()`. On a large install it should become a scheduled task with a
   resume cursor.
6. **Stats alignment.** The Statistiques screen still measures the structural
   gate ("Structure conforme") while the lists show the editorial verdict, and
   its "Jobs récents" table now duplicates the Jobs tab. Decide one vocabulary
   and remove the duplicate.
7. **Retention and privacy.** `purge_expired()` deletes events and calls by
...
- **Unaudited modules.** `class-msrwa-pipeline.php`, `class-msrwa-settings.php`,
  `class-msrwa-images.php`, `class-msrwa-storage.php` and the provider
  adapters have not had a line-by-line review. Read before trusting.
