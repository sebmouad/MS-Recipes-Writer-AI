...
msrwa_test_assert( array( 'research', 'canonical_recipe', 'article', 'featured_image', 'facebook_image', 'review', 'corrections', 'proofread', 'final_approval' ) === $engine_steps, 'The engine must expose the whole pipeline, in order; got ' . implode( ', ', $engine_steps ) );
$lab_prompts = glob( dirname( __DIR__ ) . '/includes/engine/prompts/*.txt' );
msrwa_test_assert( 12 === count( $lab_prompts ), 'One maintained prompt per step that calls a model — four text stages, two image stages and the final approval — plus the research written from the editor’s photographs, and the collage’s brief and composing instruction.' );
...
msrwa_test_contains( $article_input, 'APPEARANCE OF THE COOKED DISH', 'Real-image observations must reach the article, distilled rather than raw.' );
...
msrwa_test_contains( $facebook_prompt, 'EXACTLY 6 equal square images', 'The Facebook collage must require a strict six-panel grid.' );
