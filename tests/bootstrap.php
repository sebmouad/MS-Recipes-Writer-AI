...
if ( ! function_exists( 'delete_transient' ) ) { function delete_transient( $key ) { unset( $GLOBALS['msrwa_test_transients'][ $key ] ); return true; } }
if ( ! function_exists( 'disabled' ) ) { function disabled( $disabled, $current = true, $echo = true ) { $out = (string) $disabled === (string) $current ? " disabled='disabled'" : ''; if ( $echo ) { echo $out; } return $out; } }
