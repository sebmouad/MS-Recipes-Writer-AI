(function () {
  ...
  if (button) button.addEventListener('click', function () {
    ...
    fetch(MSRWA.api + '/batches', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-WP-Nonce': MSRWA.nonce }, body: JSON.stringify({ items: lines.map(function (title) { return { title: title }; }) }) })
      ...
      .then(function (data) { message.textContent = 'Lot #' + data.id + ' créé. Il attend la validation du budget de test.'; document.getElementById('msrwa-titles').value = ''; })
