...
final class MSRWA_DB {
	public static function tables() {
		...
		return array(
			...
			'lab_runs' => $prefix . 'lab_runs',
			'lab_steps' => $prefix . 'lab_steps',
			'lab_calls' => $prefix . 'lab_calls',
			'lab_events' => $prefix . 'lab_events',
			'lab_artifacts' => $prefix . 'lab_artifacts',
		);
	}
	...
	public static function install() {
		...
		$columns = array(
			...
			'article_quality' => "ALTER TABLE {$t['jobs']} ADD COLUMN article_quality varchar(20) NULL AFTER draft_post_id",
			'featured_quality' => "ALTER TABLE {$t['jobs']} ADD COLUMN featured_quality varchar(20) NULL AFTER article_quality",
			'facebook_quality' => "ALTER TABLE {$t['jobs']} ADD COLUMN facebook_quality varchar(20) NULL AFTER featured_quality",
		);
	...
	public static function store_article_quality( $job_id, $report ) {
		return self::store_quality_verdicts( $job_id, array( 'article_quality' => self::legacy_article_verdict( $report ) ) );
	}

	/** Persist independent AI verdicts for article information and both image outputs. */
	public static function store_quality_verdicts( $job_id, $verdicts ) {
		global $wpdb;
		$allowed = array( 'good', 'needs_review', 'bad', 'unknown', 'not_generated' );
		$row = array(); $formats = array();
		foreach ( array( 'article_quality', 'featured_quality', 'facebook_quality' ) as $key ) {
			if ( ! array_key_exists( $key, (array) $verdicts ) ) { continue; }
			$value = sanitize_key( (string) $verdicts[ $key ] );
			$row[ $key ] = in_array( $value, $allowed, true ) ? $value : 'unknown'; $formats[] = '%s';
		}
		if ( ! $row ) { return false; }
		$row['quality_checked_at'] = current_time( 'mysql', true ); $formats[] = '%s';
	...
	private static function legacy_article_verdict( $report ) {
		$report = is_array( $report ) ? $report : array();
		if ( isset( $report['article_quality'] ) ) { return $report['article_quality']; }
		return 'checks_passed' === ( $report['status'] ?? '' ) ? 'good' : 'needs_review';
	}
	...
	/** Strips secrets and binary payloads from anything on its way into the database. */
	public static function sanitize_persisted_data( $value ) {
		if ( is_string( $value ) && strlen( $value ) > 262144 ) { return substr( $value, 0, 262144 ) . '\n[truncated]'; }
		if ( ! is_array( $value ) ) { return $value; }
		$out = array();
		foreach ( $value as $key => $item ) {
			$name = (string) $key;
			if ( preg_match( '/(?:api.?key|secret|password|authorization|lock_token)/i', $name ) ) { $out[ $key ] = '[redacted]'; continue; }
			if ( in_array( strtolower( $name ), array( 'base64', 'b64_json' ), true ) && is_string( $item ) ) { $out[ $key ] = array( 'binary_omitted' => true, 'encoded_bytes' => strlen( $item ), 'sha256' => hash( 'sha256', $item ) ); continue; }
			$out[ $key ] = self::sanitize_persisted_data( $item );
		}
		return $out;
	}

	public static function diagnostic_payload( $value ) {
		if ( is_wp_error( $value ) ) { return self::sanitize_persisted_data( array( 'error_code' => $value->get_error_code(), 'error_message' => $value->get_error_message(), 'error_data' => $value->get_error_data() ) ); }
		return self::sanitize_persisted_data( $value );
	}
	...
	public static function call( $data ) {
		global $wpdb;
		$t = self::tables();
		$allowed = array( 'batch_id', 'job_id', 'provider', 'model', 'operation', 'status', 'http_status', 'request_id', 'input_tokens', 'output_tokens', 'cost_estimate', 'uncertain', 'error_code', 'request_json', 'response_json', 'payload_hash', 'started_at', 'finished_at' );
		$row = array();
		$formats = array();
		foreach ( $allowed as $key ) {
			if ( ! array_key_exists( $key, $data ) ) { continue; }
			$row[ $key ] = in_array( $key, array( 'request_json', 'response_json' ), true ) && is_array( $data[ $key ] ) ? wp_json_encode( self::sanitize_persisted_data( $data[ $key ] ), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) : $data[ $key ];
