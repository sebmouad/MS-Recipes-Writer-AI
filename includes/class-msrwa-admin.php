...
final class MSRWA_Admin {
	...
	public static function page() {
		...
		<?php $stats = MSRWA_Stats::summary( 7, current_user_can( 'msrwa_view_all' ) || current_user_can( 'manage_options' ) ? 0 : get_current_user_id() ); ?><div class="wrap msrwa-wrap"><h1>MS Recipes Writer AI</h1><section class="msrwa-card"><h2>Activité des 7 derniers jours</h2><div class="msrwa-fields"><div><strong><?php echo esc_html( $stats['total_jobs'] ); ?></strong><span>jobs</span></div><div><strong><?php echo esc_html( $stats['estimated_cost_usd'] ); ?> $</strong><span>coût estimé</span></div><div><strong><?php echo esc_html( array_sum( array_column( $stats['calls'], 'count' ) ) ); ?></strong><span>appels</span></div></div></section>
			<div class="msrwa-grid"><section class="msrwa-card"><h2>Nouveau lot</h2><p>Une recette par ligne. Les champs recette et images sont facultatifs et associés automatiquement par position.</p><label>Titres<textarea id="msrwa-titles" rows="6" placeholder="Titre de recette"></textarea></label><label>Texte ou recette fournie<textarea id="msrwa-recipes" rows="6" placeholder="Optionnel — une recette par ligne"></textarea></label><label>Images de référence<textarea id="msrwa-images" rows="4" placeholder="Optionnel — URLs séparées par des espaces, une recette par ligne"></textarea></label><button class="button button-primary" id="msrwa-create">Créer le lot</button><p id="msrwa-message" role="status"></p></section>
			...
			<?php global $wpdb; $table_names = MSRWA_DB::tables(); $owner_filter = current_user_can( 'msrwa_view_all' ) || current_user_can( 'manage_options' ) ? '' : $wpdb->prepare( ' WHERE owner_id = %d', get_current_user_id() ); $recent_batches = $wpdb->get_results( "SELECT id,status,total,completed,created_at,updated_at FROM {$table_names['batches']} {$owner_filter} ORDER BY id DESC LIMIT 20", ARRAY_A ); ?><section class="msrwa-card"><h2>Lots récents</h2><table class="widefat striped"><thead><tr><th>ID</th><th>État</th><th>Progression</th><th>Créé</th><th>Mis à jour</th></tr></thead><tbody><?php if ( empty( $recent_batches ) ) : ?><tr><td colspan="5">Aucun lot.</td></tr><?php else : foreach ( $recent_batches as $batch ) : ?><tr><td><?php echo esc_html( $batch['id'] ); ?></td><td><?php echo esc_html( $batch['status'] ); ?></td><td><?php echo esc_html( $batch['completed'] . ' / ' . $batch['total'] ); ?></td><td><?php echo esc_html( $batch['created_at'] ); ?></td><td><?php echo esc_html( $batch['updated_at'] ); ?></td></tr><?php endforeach; endif; ?></tbody></table></section>
		</div><?php
	}
