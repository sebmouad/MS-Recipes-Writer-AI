...
final class MSRWA_Admin {
	...
	private static function render_editorial_report( $report ) {
		...
		foreach ( array( 'article_quality' => 'Contenu', 'featured_quality' => 'Image principale', 'facebook_quality' => 'Image Facebook' ) as $key => $label ) { echo '<p>' . esc_html( $label ) . ' : '; MSRWA_Presentation::render_verdict( $report[ $key ] ?? 'unknown' ); echo '</p>'; }
		foreach ( (array) ( $report['summaries'] ?? array() ) as $summary ) { if ( is_string( $summary ) && $summary ) { echo '<p>' . esc_html( $summary ) . '</p>'; } }
	...
	/**
	 * Tells editors when nothing is progressing, so a stalled scheduler is not
	 * read as a plugin failure. Read-only: visiting a screen never runs a job.
	 */
	private static function render_queue_notice() {
		$health = MSRWA_Queue::health();
		if ( 'healthy' === $health['status'] && ! $health['expired_workers'] ) { return; }
		$is_admin = current_user_can( 'manage_options' );
		$messages = array();
		if ( $health['expired_workers'] ) { $messages[] = sprintf( '%d traitement(s) interrompu(s) sont repris automatiquement.', (int) $health['expired_workers'] ); }
		if ( 'warning' === $health['status'] ) { $messages[] = sprintf( '%d job(s) en attente et aucune tâche planifiée : la planification WordPress semble arrêtée.', (int) $health['waiting'] ); }
		if ( ! $messages ) { return; }
		echo '<div class="notice notice-warning msrwa-queue-notice"><p><strong>File d’attente</strong> — ' . esc_html( implode( ' ', $messages ) ) . '</p>';
		if ( $is_admin ) {
			echo '<p class="description">Moteur : ' . esc_html( $health['driver'] ) . ' · en attente : ' . esc_html( number_format_i18n( $health['waiting'] ) ) . ' · en cours : ' . esc_html( number_format_i18n( $health['running'] ) ) . ' · à vérifier : ' . esc_html( number_format_i18n( $health['needs_attention'] ) ) . '. <a href="' . esc_url( admin_url( 'admin.php?page=ms-recipes-writer-ai-settings' ) ) . '">Diagnostic de la file</a></p>';
		}
		echo '</div>';
	}

	/** Job actions the current status actually allows, for the list rows. */
	private static function render_job_actions( $row ) {
		$retryable = in_array( $row['status'], array( 'failed', 'needs_review', 'awaiting_input', 'uncertain', 'paused_budget', 'paused', 'awaiting_admin' ), true );
		$cancellable = ! in_array( $row['status'], array( 'completed', 'cancelled', 'canceled' ), true );
		$association = 'awaiting_input' === $row['status'] && 'association' === $row['stage'];
		if ( ! $retryable && ! $cancellable ) { return; }
		echo '<p class="msrwa-job-row-actions">';
		if ( $association ) { echo '<button type="button" class="button button-primary msrwa-job-action" data-action="association" data-job-id="' . esc_attr( $row['id'] ) . '">Confirmer l’association</button> '; }
		if ( $retryable ) { echo '<button type="button" class="button msrwa-job-action" data-action="retry" data-job-id="' . esc_attr( $row['id'] ) . '">Relancer</button> '; }
		if ( $cancellable ) { echo '<button type="button" class="button msrwa-job-action" data-action="cancel" data-job-id="' . esc_attr( $row['id'] ) . '">Annuler</button> '; }
		echo '<span class="msrwa-job-action-status" role="status" aria-live="polite"></span></p>';
	}
	...
	private static function render_filters( $args, $result ) {
		...
		<form class="msrwa-filters" method="get" action="<?php echo esc_url( admin_url( 'admin.php' ) ); ?>">
			...
			if ( 'articles' === $args['view'] ) {
				...
				self::render_select( 'msrwa_quality', 'Qualité contenu', array_intersect_key( MSRWA_Lists::quality_labels(), array_flip( array( 'good', 'review', 'bad', 'pending' ) ) ), $args['quality'], 'Toutes qualités' );
			} else {
	...
	private static function render_articles_list( $args ) {
		...
		$can_view_all = MSRWA_Lists::can_view_all();
		$columns = $can_view_all ? 14 : 13;
		?>
		<section class="msrwa-card"><div class="msrwa-section-head"><div><h2>Articles</h2><p class="description">Verdicts IA distincts pour le contenu et le réalisme de chaque image. <?php echo esc_html( $can_view_all ? 'Tous les éditeurs sont visibles ; utilisez le filtre Auteur.' : 'Vous voyez uniquement vos articles.' ); ?></p></div></div>
		<?php self::render_filters( $args, $result ); ?>
		<div class="msrwa-table-scroll"><table class="widefat striped"><thead><tr><th>Article</th><th>Publication</th><th>Contenu</th><th>Image principale</th><th>Image Facebook</th><th>État</th><?php if ( $can_view_all ) : ?><th>Auteur</th><?php endif; ?><th>Lot</th><th>Coût</th><th>Mis à jour</th><th>Actions</th></tr></thead><tbody>
		...
		?>
			<tr>
				...
				<td class="msrwa-quality-cell"><?php MSRWA_Presentation::render_verdict( $row['article_quality'] ?? '' ); ?></td>
				<td class="msrwa-quality-cell"><?php MSRWA_Presentation::render_verdict( $row['featured_quality'] ?? '' ); ?></td>
				<td class="msrwa-quality-cell"><?php MSRWA_Presentation::render_verdict( $row['facebook_quality'] ?? '' ); ?></td>
	...
	private static function render_jobs_list( $args, $settings ) {
		...
		<?php self::render_filters( $args, $result ); ?>
		<div class="msrwa-table-scroll"><table class="widefat striped"><thead><tr><th>Job</th><?php if ( $can_view_all ) : ?><th>Auteur</th><?php endif; ?><th>Lot</th><th>État</th><th>Étape</th><th>Contenu</th><th>Image principale</th><th>Image Facebook</th><th>Tentatives</th><th>Corrections</th><th>Appels</th><th>Tokens</th><th>Coût</th><th>Durée</th></tr></thead><tbody>
	...
	public static function job_detail_page() {
		...
		<div class="wrap msrwa-wrap msrwa-job-debug">
			...
			<section class="msrwa-kpis msrwa-stats-kpis"><div><span>État</span><strong><?php echo esc_html( MSRWA_Presentation::state( $job['status'] ) ); ?></strong></div><?php foreach ( array( 'article_quality' => 'Contenu', 'featured_quality' => 'Image principale', 'facebook_quality' => 'Image Facebook' ) as $field => $label ) : ?><div><span><?php echo esc_html( $label ); ?></span><?php MSRWA_Presentation::render_verdict( $job[ $field ] ?? 'unknown' ); ?></div><?php endforeach; ?><div><span>Étape</span><strong><?php echo esc_html( $job['stage'] ); ?></strong></div><div><span>Coût</span><strong><?php echo esc_html( number_format_i18n( (float) $job['cost_estimate'], 4 ) ); ?> $</strong></div><div><span>Tokens</span><strong><?php echo esc_html( number_format_i18n( array_sum( wp_list_pluck( $calls, 'input_tokens' ) ) + array_sum( wp_list_pluck( $calls, 'output_tokens' ) ) ) ); ?></strong></div><div><span>Corrections</span><strong><?php echo esc_html( $job['correction_cycles'] ); ?></strong></div><div><span>Exécutions</span><strong><?php echo esc_html( $job['attempts'] ); ?></strong></div><div><span>Retries étape</span><strong><?php echo esc_html( $job['retry_attempts'] ); ?>/3</strong></div></section>
	...
	private static function render_stats_details( $details, $show_editors ) {
		...
		<div class="wrap msrwa-wrap msrwa-stats-details">
			...
			<section class="msrwa-kpis msrwa-stats-kpis"><div><span>Articles produits</span><strong><?php echo esc_html( number_format_i18n( (int) ( $quality['articles'] ?? 0 ) ) ); ?></strong></div><div><span>Articles validés par IA</span><strong><?php echo esc_html( number_format_i18n( (int) ( $quality['approved'] ?? 0 ) ) ); ?></strong></div><div><span>À vérifier par IA</span><strong><?php echo esc_html( number_format_i18n( max( 0, (int) ( $quality['articles'] ?? 0 ) - (int) ( $quality['approved'] ?? 0 ) ) ) ); ?></strong></div><div><span>Articles sous coût cible</span><strong><?php echo esc_html( number_format_i18n( (int) ( $quality['within_target_cost'] ?? 0 ) ) ); ?></strong></div><div><span>Coût cible</span><strong><?php echo esc_html( number_format_i18n( (float) ( $quality['target_cost_usd'] ?? 0.10 ), 2 ) ); ?> $</strong></div></section>
			...
			<section class="msrwa-card"><div class="msrwa-section-head"><div><h2>Jobs récents</h2><p class="description">Aperçu des 25 derniers jobs. La liste complète, filtrable par auteur, état et étape, se trouve dans l’onglet Jobs.</p></div><div><a class="button" href="<?php echo esc_url( add_query_arg( array( 'page' => 'ms-recipes-writer-ai', 'msrwa_view' => 'jobs' ), admin_url( 'admin.php' ) ) ); ?>">Ouvrir la liste des jobs</a></div></div><div class="msrwa-table-scroll"><table class="widefat striped"><thead><tr><th>Job</th><th>État</th><th>Contenu</th><th>Image principale</th><th>Image Facebook</th><th>Corrections</th><th>Coût</th><th>Durée</th><th></th></tr></thead><tbody><?php if ( empty( $details['recent_jobs'] ) ) : ?><tr><td colspan="9">Aucun job.</td></tr><?php else : foreach ( $details['recent_jobs'] as $job ) : $duration = max( 0, strtotime( $job['updated_at'] ) - strtotime( $job['created_at'] ) ); ?><tr><td>#<?php echo esc_html( $job['id'] ); ?> — <?php echo esc_html( $job['title'] ); ?></td><td><?php echo esc_html( MSRWA_Presentation::state( $job['status'] ) ); ?></td><?php foreach ( array( 'article_quality', 'featured_quality', 'facebook_quality' ) as $field ) : ?><td><?php MSRWA_Presentation::render_verdict( $job[ $field ] ?? 'unknown' ); ?></td><?php endforeach; ?><td><?php echo esc_html( $job['correction_cycles'] ); ?></td><td><?php echo esc_html( number_format_i18n( (float) $job['cost_estimate'], 4 ) ); ?> $</td><td><?php echo esc_html( human_time_diff( 0, $duration ) ); ?></td><td><a class="button" href="<?php echo esc_url( add_query_arg( array( 'page' => 'ms-recipes-writer-ai-job-detail', 'job_id' => $job['id'] ), admin_url( 'admin.php' ) ) ); ?>">Détails</a></td></tr><?php endforeach; endif; ?></tbody></table></div></section>
		</div>
		<?php
	}
	...
	private static function render_execution_settings( $s ) {
		...
		$fields = array( 'web_search_max_tool_calls' => 'Recherche web : appels maximum par requête', 'research_max_output_tokens' => 'Recherche : tokens de sortie', 'association_max_output_tokens' => 'Association : tokens de sortie', 'canonical_max_output_tokens' => 'Recette : tokens de sortie', 'router_max_output_tokens' => 'Sélection des modèles : tokens de sortie', 'vision_max_output_tokens' => 'Analyse visuelle : tokens de sortie', 'image_review_max_output_tokens' => 'Relecture image : tokens de sortie', 'text_reserve_margin_usd' => 'Marge de réservation texte (USD)' );
	...
	public static function settings_page() {
		...
		?><div class="wrap msrwa-wrap"><h1>MS Recipes Writer AI — Configuration</h1><p class="description">Gérez les modèles, prompts, coûts, limites et règles qualité.</p><nav class="msrwa-range-tabs" aria-label="Sections"><a href="#msrwa-settings-output">Pagination et images</a><a href="#msrwa-settings-quality">Qualité</a><a href="#msrwa-settings-providers">Fournisseurs et modèles</a><a href="#msrwa-settings-costs">Budgets</a><a href="#msrwa-settings-history">Historique</a></nav><?php if ( isset( $_GET['updated'] ) ) : ?><div class="notice notice-success is-dismissible"><p>Configuration enregistrée.</p></div><?php endif; ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="msrwa_save_settings"><?php wp_nonce_field( 'msrwa_save_settings' ); ?><div class="msrwa-card"><h2>Mode et limites</h2><label>Mode imposé<select name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[mode]"><option value="automatic" <?php selected( $s['mode'], 'automatic' ); ?>>Automatique — équilibre qualité/coût</option><option value="manual" <?php selected( $s['mode'], 'manual' ); ?>>Manuel — réglages administrateur</option></select></label><div class="msrwa-fields"><label>Lot maximum<input type="number" min="1" max="50" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[max_batch]" value="<?php echo esc_attr( $s['max_batch'] ); ?>"></label><label>Simultané maximum<input type="number" min="1" max="4" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[max_concurrency]" value="<?php echo esc_attr( $s['max_concurrency'] ); ?>"></label><label>Corrections maximum<input type="number" min="0" max="2" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[max_corrections]" value="<?php echo esc_attr( $s['max_corrections'] ); ?>"></label></div></div>
		<section class="msrwa-card" id="msrwa-settings-output"><h2>Pagination et images</h2><h3>Pagination des articles</h3><p><label><input type="checkbox" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[article_pagination_enabled]" value="1" <?php checked( $s['article_pagination_enabled'], 1 ); ?>> Diviser les articles en deux pages WordPress</label></p><div class="msrwa-fields"><label>Mots minimum pour division<input type="number" min="300" max="8000" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[article_pagination_min_words]" value="<?php echo esc_attr( $s['article_pagination_min_words'] ); ?>"></label><label>Position division (%)<input type="number" min="30" max="70" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[article_pagination_split_percent]" value="<?php echo esc_attr( $s['article_pagination_split_percent'] ); ?>"></label></div><p class="description">Division WordPress en deux pages à une limite de paragraphe ou de section. Désactivée : article sur une seule page.</p><h3>Images à générer</h3><?php foreach ( array( 'generate_featured_image' => 'Générer l’image principale (Featured)', 'generate_facebook_image' => 'Générer l’image Facebook' ) as $key => $label ) : ?><label><input type="checkbox" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD . '[' . $key . ']' ); ?>" value="1" <?php checked( $s[ $key ], 1 ); ?>> <?php echo esc_html( $label ); ?></label><?php endforeach; ?><p class="description">Choix indépendants. Une image désactivée ne déclenche ni génération ni contrôle image. Facebook seul : création depuis la recette, sans image principale. Les choix sont conservés avec chaque nouveau job.</p></section><div class="msrwa-card" id="msrwa-settings-quality"><h2>Cadre éditorial</h2><p class="description">Objectifs de rédaction. La qualité est évaluée séparément par les relecteurs IA.</p><div class="msrwa-fields"><label>Mots minimum<input type="number" min="300" max="8000" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[quality_min_words]" value="<?php echo esc_attr( $s['quality_min_words'] ); ?>"></label><label>Mots maximum<input type="number" min="500" max="10000" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[quality_max_words]" value="<?php echo esc_attr( $s['quality_max_words'] ); ?>"></label><label>Titres h2/h3 minimum<input type="number" min="3" max="80" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[quality_min_headings]" value="<?php echo esc_attr( $s['quality_min_headings'] ); ?>"></label><label>Paragraphes minimum<input type="number" min="5" max="150" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[quality_min_paragraphs]" value="<?php echo esc_attr( $s['quality_min_paragraphs'] ); ?>"></label><label>Ingrédients indicatifs<input type="number" min="1" max="50" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[quality_min_ingredients]" value="<?php echo esc_attr( $s['quality_min_ingredients'] ); ?>"></label><label>Étapes indicatives<input type="number" min="1" max="40" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[quality_min_steps]" value="<?php echo esc_attr( $s['quality_min_steps'] ); ?>"></label><label>Sortie article max (tokens)<input type="number" min="1000" max="20000" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[article_max_output_tokens]" value="<?php echo esc_attr( $s['article_max_output_tokens'] ); ?>"></label><label>Sortie relecture max (tokens)<input type="number" min="500" max="10000" name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[review_max_output_tokens]" value="<?php echo esc_attr( $s['review_max_output_tokens'] ); ?>"></label></div><p class="description">Objectifs actuels : <?php echo esc_html( number_format_i18n( $benchmark['words'] ) ); ?> mots, <?php echo esc_html( number_format_i18n( $benchmark['headings'] ) ); ?> titres et <?php echo esc_html( number_format_i18n( $benchmark['paragraphs'] ) ); ?> paragraphes minimum.</p></div>
		...
		<div class="msrwa-card"><h2>Sorties image</h2><div class="msrwa-fields"><label>Ratio image principale<select name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[featured_ratio]"><?php foreach ( array( '1:1', '4:5', '3:2', '2:3' ) as $ratio ) : ?><option value="<?php echo esc_attr( $ratio ); ?>" <?php selected( $s['featured_ratio'], $ratio ); ?>><?php echo esc_html( $ratio ); ?></option><?php endforeach; ?></select></label><label>Ratio Facebook<select name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[facebook_ratio]"><?php foreach ( array( '4:5', '1:1', '2:3', '3:2' ) as $ratio ) : ?><option value="<?php echo esc_attr( $ratio ); ?>" <?php selected( $s['facebook_ratio'], $ratio ); ?>><?php echo esc_html( $ratio ); ?></option><?php endforeach; ?></select></label><label>Qualité image principale<select name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[featured_image_quality]"><?php foreach ( MSRWA_Images::qualities() as $quality ) : ?><option value="<?php echo esc_attr( $quality ); ?>" <?php selected( MSRWA_Images::quality( $s, 'featured' ), $quality ); ?>><?php echo esc_html( $quality ); ?></option><?php endforeach; ?></select></label><label>Qualité image Facebook<select name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[facebook_image_quality]"><?php foreach ( MSRWA_Images::qualities() as $quality ) : ?><option value="<?php echo esc_attr( $quality ); ?>" <?php selected( MSRWA_Images::quality( $s, 'facebook' ), $quality ); ?>><?php echo esc_html( $quality ); ?></option><?php endforeach; ?></select></label><label>Format de sortie OpenAI<select name="<?php echo esc_attr( MSRWA_Settings::FORM_FIELD ); ?>[image_format]"><?php foreach ( array( 'webp', 'jpeg', 'png' ) as $format ) : ?><option value="<?php echo esc_attr( $format ); ?>" <?php selected( $s['image_format'], $format ); ?>><?php echo esc_html( $format ); ?></option><?php endforeach; ?></select></label></div><p class="description">Chaque image a sa propre qualité, car elles ne coûtent pas la même chose : mesuré le 20/09/2026 sur <code>gpt-image-2.5-flare</code> en 1024×1024, une image principale revient à 0,0104 $ en <code>low</code>, 0,0177 $ en <code>medium</code> et 0,0572 $ en <code>high</code>. Les formats et qualités natifs dépendent du fournisseur. Le recadrage conserve le fichier final dans le format choisi sans étirer l’image.</p></div>
