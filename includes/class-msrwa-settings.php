...
- At most 10 ingredient facts, 12 preparation facts and 6 references. At most 4 substitutions, 4 accompaniments, 5 common failures, 3 storage facts, 3 food safety facts, 4 uncertainties and 3 originality notes; every value one short sentence. The whole package fits in about 2,500 words: the recipe step builds on it, it is not the article.
...
4. THE COLLAGE AS A SEQUENCE — only when you were sent the collage. It must read as one recipe being made, in the order the recipe makes it. Blocking: a step shown before a step that must precede it, a panel that repeats another, a panel showing something the recipe never does, a panel whose subject is equipment rather than food — baking paper, baking beans or weights, foil — even when the recipe uses it, or a blank or empty cell. The number of panels and their styling are otherwise minor. How many pieces a panel shows is a count, and counts are minor here too: four shanks on the board where the recipe has six, or a bowl that seems to hold more potatoes than listed, is never blocking — the mise en place is an impression of the ingredients, and the quantities are in the recipe.
...
1. Mise en place: only the canonical raw ingredients, in the forms the recipe uses, with the empty cookware nearby; the principal ingredient dominant; no packaging. Nothing already made: no pastry rolled out or lined in its tin, nothing cut, cooked or assembled that a later panel shows being made.
...
ORDER IS THE RECIPE\'S. Map every panel to its step number and lay them out in ascending order; move or replace any moment that would come before a step that must precede it. Every panel shows the food in the state all earlier steps leave it in, shown or not: a case baked before filling is visibly part-baked when the filling goes in; meat browned before braising goes into the liquid browned. Give each cooking stage — every trip to the oven, pan or pot — its own panel before any preparation moment, merging preparation moments to make room, and show it by its result, never by the equipment that is taken away afterwards: a case baked blind is the golden, dry, empty case after its bake. No ingredient appears before the step that adds it: olives added after the chicken returns to the pot are not in the marinade, a garnish added at serving is not in the oven. Leave out passive moments — preheating, waiting, chilling — unless they change what is visible. Never invent a step, ingredient, garnish or method. Before returning, read the panels in order and fix any that could not follow the one before it.
...
FORBIDDEN anywhere, in any panel, even as a prop: text, letters, numbers, captions, labels, logos, watermark, packaging, branded containers, oven display, arrows, badges, stickers; baking paper liners, baking beans, pie weights and foil.',
		);
