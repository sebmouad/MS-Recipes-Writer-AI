...
final class MSRWA_Stats {
	...
	public static function details( $days = 7, $owner_id = 0 ) {
		...
		$recent_sql = "SELECT j.id,j.batch_id,j.title,j.status,j.stage,j.artifacts_json,j.draft_post_id,j.cost_estimate,j.correction_cycles,j.attempts,j.retry_attempts,j.created_at,j.updated_at,a.content_json AS quality_json FROM {$t['jobs']} j LEFT JOIN {$t['artifacts']} a ON a.job_id = j.id AND a.artifact_key = 'quality_report' AND a.status = 'current' WHERE {$job_where} ORDER BY j.id DESC LIMIT 25";
